        ...
        <div className="relative px-6 pt-6 pb-7 space-y-4">
          <DialogHeader>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Download className="w-4 h-4" style={{ color: 'rgba(230,161,71,0.7)' }} />
                <DialogTitle
              ...
              >
                <Crown className="w-2.5 h-2.5" style={{ color: 'rgba(230,161,71,0.8)' }} />
                <span className="text-[8px] font-mono font-bold tracking-wider" style={{ color: 'rgba(230,161,71,0.8)' }}>PRO</span>
              </div>
            </div>
          </DialogHeader>
          ...
          >
            <div className="flex items-center gap-1.5">
              <span className="font-bold" style={{ color: 'rgba(230,161,71,0.9)' }}>{storeWallets.length}</span>
              <span style={{ color: 'rgba(255,255,255,0.3)' }}>wallet{storeWallets.length !== 1 ? 's' : ''}</span>
            </div>
            <span style={{ color: 'rgba(255,255,255,0.12)' }}>·</span>
            <div className="flex items-center gap-1.5">
              <span className="font-bold" style={{ color: 'rgba(230,161,71,0.9)' }}>{totalTxs}</span>
            ...
            {EXPORTS.map(({ id, label, desc, icon: Icon, fn }) => {
              ...
              return (
                <button
                  ...
                  style={{
                    background: isDone ? 'rgba(230,161,71,0.06)' : 'rgba(255,255,255,0.025)',
                    border:     `1px solid ${isDone ? 'rgba(230,161,71,0.25)' : 'rgba(255,255,255,0.065)'}`,
                  }}
                  onMouseEnter={e => {
                    if (!loading && !isDone) {
                ...
                >
                  {/* Icon */}
                  {isDone ? (
                    <CheckCircle2 className="w-4 h-4 mt-0.5 flex-shrink-0" style={{ color: 'rgba(230,161,71,0.8)' }} />
                  ) : (
                    <Icon
                      className="w-4 h-4 mt-0.5 flex-shrink-0"
                      style={{ color: isLoading ? 'rgba(230,161,71,0.6)' : 'rgba(255,255,255,0.3)' }}
                    />
                  )}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-medium" style={{ color: isDone ? 'rgba(230,161,71,0.9)' : 'rgba(255,255,255,0.8)' }}>
                        {label}
                      </span>
                      {isLoading && (
                        <span className="text-[9px] font-mono animate-pulse" style={{ color: 'rgba(230,161,71,0.7)' }}>
                          generating…
                        </span>
                      )}
                      {isDone && (
                        <span className="text-[9px] font-mono" style={{ color: 'rgba(230,161,71,0.7)' }}>
                          ✓ downloaded
                        </span>
                      )}
                    </div>
