...
const Index = () => {
  ...
  // ── ENS Resolution — auto-resolve ETH addresses to ENS names ───────────────
  const walletAddresses = useMemo(() => storeWallets.map(w => w.address), [storeWallets])
  const ensMap          = useEnsResolver(walletAddresses)

  const allWallets = useMemo<Wallet[]>(
    () => storeWallets.map((w, i) => {
      const ensName = ensMap[w.address.toLowerCase()]
      const walletWithEns = ensName ? { ...w, label: ensName } : w
      return toUiWallet(walletWithEns, apiDataArr?.[i], i)
    }),
    [storeWallets, apiDataArr, ensMap]
  )
  ...
  const walletLabels = useMemo<Record<string, string>>(() => {
    ...
    storeWallets.forEach(w => {
      // Use ENS name if resolved, otherwise use stored label
      const ens = ensMap[w.address.toLowerCase()]
      m[w.id] = ens ?? w.label
    })
    return m
  }, [storeWallets, ensMap])
